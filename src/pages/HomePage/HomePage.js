...
const HomePage = () => {
  ...
  const showSection = (products) => {
    return searchQuery ? filteredProducts(products).length > 0 : true;
  };
    ...
    <div className="home-page">
      ...
      <main>
        <div className="hero-section">
          ...
          >
            <div>
              <img src="https://res.cloudinary.com/dplera1gb/image/upload/v1719341327/satoyu-productlist-banner_oozndf.png" alt="Slide 1" />
            </div>
            <div>
              <img src="https://res.cloudinary.com/dplera1gb/image/upload/v1719341081/cheese-slide-scaled_yhys4d.webp" alt="Slide 2" />
            </div>
            <div>
              <img src="https://res.cloudinary.com/dplera1gb/image/upload/v1719341444/pngtree-various-bakery-items-are-displayed-on-a-wooden-table-picture-image_2656185-transformed_eg9bel.jpg" alt="Slide 3" />
            </div>
          </Carousel>
        </div>
        {!searchQuery && (
        ...
        {showSection(sampleProducts) && (
          <div className="product-list">
            <h2>Featured Products</h2>
            <div className="products">
              {filteredProducts(sampleProducts).map((product) => (
                <ProductCard key={product.id} product={product} />
              ))}
            </div>
          </div>
        )}
        {showSection(fruitProducts) && (
          <div ref={fruitsSectionRef} className="product-list">
            <h2>Fruits</h2>
            <div className="products">
              {filteredProducts(fruitProducts).map((product) => (
                <ProductCard key={product.id} product={product} />
              ))}
            </div>
          </div>
        )}
        {showSection(dairyProducts) && (
          <div ref={dairySectionRef} className="product-list">
            <h2>Dairy</h2>
            <div className="products">
              {filteredProducts(dairyProducts).map((product) => (
                <ProductCard key={product.id} product={product} />
              ))}
            </div>
          </div>
        )}
        {showSection(bakeryProducts) && (
          <div ref={bakerySectionRef} className="product-list">
            <h2>Bakery</h2>
            <div className="products">
              {filteredProducts(bakeryProducts).map((product) => (
                <ProductCard key={product.id} product={product} />
              ))}
            </div>
          </div>
